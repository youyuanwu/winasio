...
// pipe for server
template <typename Executor = boost::asio::any_io_executor>
class server_named_pipe
    : public boost::asio::windows::basic_stream_handle<Executor> {
public:
  ...
  server_named_pipe(executor_type &ex)
    ...
    if (pipe::is_open()) {
      ...
      if (!DisconnectNamedPipe(pipe::native_handle())) {
        boost::system::error_code ec = boost::system::error_code(
            static_cast<int>(GetLastError()), boost::system::system_category());
...
template <typename Executor = boost::asio::any_io_executor>
class client_named_pipe
    : public boost::asio::windows::basic_stream_handle<Executor> {
public:
  typedef Executor executor_type;
  typedef std::string endpoint_type;

  client_named_pipe(executor_type &ex)
      : boost::asio::windows::basic_stream_handle<executor_type>(ex) {}
