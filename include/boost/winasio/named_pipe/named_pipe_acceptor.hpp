...
#include "boost/winasio/named_pipe/named_pipe_server_details.hpp"
...
namespace boost
{
    namespace winasio
    {
        ...
        public:
            ...
            async_accept(named_pipe<executor_type> & pipe, BOOST_ASIO_MOVE_ARG(AcceptToken) token BOOST_ASIO_DEFAULT_COMPLETION_TOKEN(executor_type))
            {

                return boost::asio::async_compose<AcceptToken, void(boost::system::error_code)>(
                    details::async_accept_op<executor_type>(&pipe, this->endpoint_),
                    token,
                    pipe);
            }
            ...
            BOOST_ASIO_INITFN_AUTO_RESULT_TYPE(MoveAcceptToken, void(boost::system::error_code, named_pipe<executor_type>))
                             ...
                             token BOOST_ASIO_DEFAULT_COMPLETION_TOKEN(executor_type))
            {
                ...
                return boost::asio::async_compose<MoveAcceptToken, void(boost::system::error_code, named_pipe<executor_type>)>(
                    details::async_move_accept_op<executor_type>(&this->pipe_, this->endpoint_),
                    token,
                    this->pipe_);
            }
