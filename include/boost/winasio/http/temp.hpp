...
public:
    ...
    http_connection(
        ...
        : http_connection(queue_handle, [](const winnet::http::simple_request & request, winnet::http::simple_response & response)
                          {
                          // default handler
    ...
    http_connection(
        winnet::http::basic_http_queue_handle<executor_type> & queue_handle,
        std::function<void(const winnet::http::simple_request &, winnet::http::simple_response &)>
            handler)
        ...
        winnet::http::async_receive(
            ...
            [self](boost::system::error_code ec, std::size_t)
            {
                ...
                else
                {
                    ...
                    std::make_shared<http_connection>(self->queue_handle_, self->handler_)
                        ->start();
                }
            });
    }
