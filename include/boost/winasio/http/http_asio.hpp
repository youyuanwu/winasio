    ...
    namespace winasio
    {
        namespace http
        {
            ...
            namespace details
            {
                ...
                public:
                    typedef Executor executor_type;
                    async_receive_body_op(basic_http_queue_handle<executor_type> & h, HTTP_REQUEST_ID id, DynamicBuffer & buff)
                        : h_(h)
                        , id_(id)
                        , buff_(buff)
                        , state_(state::idle)
                ...
                private:
                    ...
                    template <typename Self>
                    void recieve_body(Self & self, std::size_t len)
                    {
                        auto mutbuff = buff_.prepare(len);
                        h_.async_recieve_body(id_,
                                              0, // flag
                                              (PVOID) mutbuff.data(),
                                              static_cast<ULONG>(mutbuff.size()),
                                              std::move(self));
                    }
                };
            ...
            auto async_receive(basic_http_queue_handle<Executor> & h, DynamicBuffer & buffer, Token && token)
            {

                return boost::asio::async_compose<Token, void(boost::system::error_code, std::size_t)>(
                    details::async_receive_op<Executor, DynamicBuffer>(h, buffer),
                    token,
                    h);
            }
            ...
            // async recieve body
            template <typename Executor, typename DynamicBuffer, BOOST_ASIO_COMPLETION_TOKEN_FOR(void(boost::system::error_code, std::size_t)) Token BOOST_ASIO_DEFAULT_COMPLETION_TOKEN_TYPE(Executor)>
            auto async_receive_body(basic_http_queue_handle<Executor> & h, HTTP_REQUEST_ID id, DynamicBuffer & buffer, Token && token)
            {
