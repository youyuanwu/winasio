...
namespace boost
{
    namespace winasio
    {
        namespace winhttp
        {
            ...
            template <typename Executor>
            void __stdcall BasicAsioAsyncCallback(HINTERNET hInternet, DWORD_PTR dwContext, DWORD dwInternetStatus, LPVOID lpvStatusInformation, DWORD dwStatusInformationLength)
            {
            ...
            public:
                ...
                explicit basic_winhttp_request_asio_handle(const executor_type & ex)
                    : basic_winhttp_request_handle<executor_type>(ex)
                    , ctx_(ex)
                {
                    // set call back
                }
                ...
                void managed_open(HINTERNET hConnect, LPCWSTR pwszVerb, LPCWSTR pwszObjectName, LPCWSTR pwszVersion, LPCWSTR pwszReferrer, LPCWSTR * ppwszAcceptTypes, DWORD dwFlags, boost::system::error_code & ec)
                {
                    this->open(hConnect, pwszVerb, pwszObjectName, pwszVersion, pwszReferrer, ppwszAcceptTypes, dwFlags, ec);
                    if (ec)
                    {
                        return;
                    }
                ...
                void managed_open(HINTERNET hConnect, LPCWSTR method, LPCWSTR path, boost::system::error_code & ec)
                {
                ...
                void async_send(LPCWSTR lpszHeaders, DWORD dwHeadersLength, LPVOID lpOptional, DWORD dwOptionalLength, DWORD dwTotalLength, Handler && token)
                {
                    boost::system::error_code ec;
                    // set callback in ctx.
                    ...
                    parent_type::send(lpszHeaders, dwHeadersLength, lpOptional, dwOptionalLength, dwTotalLength, (DWORD_PTR) &ctx_, ec);
                    if (ec)
                    {
                ...
                // size_t param is not used
                template <typename Handler>
                void async_recieve_response(Handler && token)
                {
                    boost::system::error_code ec;
                ...
                template <typename Handler>
                void async_query_data_available(Handler && token)
                {
                    boost::system::error_code ec;
                ...
                template <typename Handler>
                void async_read_data(_Out_ LPVOID lpBuffer, _In_ DWORD dwNumberOfBytesToRead, Handler && token)
                {
                    boost::system::error_code ec;
            ...
            namespace details
            {
                ...
                public:
                    typedef Executor executor_type;
                    async_read_body_op(basic_winhttp_request_asio_handle<executor_type> & h, DynamicBuffer & buff)
                        : h_request_(h)
                        , buff_(buff)
                        , state_(state::idle)
                    {
                    ...
                    void operator()(Self & self, boost::system::error_code ec = {}, std::size_t len = 0)
                    {
                        if (ec)
                        {
                        ...
                        case state::query_data:
                        {
                            ...
                            h_request_.async_read_data((LPVOID) buff.data(), static_cast<DWORD>(len), std::move(self));
                        }
                        break;
            ...
            template <typename Executor, typename DynamicBuffer, BOOST_ASIO_COMPLETION_TOKEN_FOR(void(boost::system::error_code, std::size_t)) Token BOOST_ASIO_DEFAULT_COMPLETION_TOKEN_TYPE(Executor)>
            auto async_read_body(basic_winhttp_request_asio_handle<Executor> & h, DynamicBuffer & buffer, Token && token)
            {
                return boost::asio::async_compose<Token, void(boost::system::error_code, std::size_t)>(
                    details::async_read_body_op<Executor, DynamicBuffer>(h, buffer),
                    token,
                    h.get_executor());
            }
