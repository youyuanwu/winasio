    ...
    namespace winasio
    {
        namespace winhttp
        {
            ...
            public:
                ...
                inline void crack(LPCWSTR pwszUrl, DWORD dwUrlLength, DWORD dwFlags, boost::system::error_code & ec)
                {
            ...
            public:
                ...
                set_status_callback(_In_ WINHTTP_STATUS_CALLBACK lpfnInternetCallback, _In_ DWORD dwNotificationFlags, _Inout_ boost::system::error_code & ec)
                {
                ...
                WINHTTP_STATUS_CALLBACK
                set_status_callback(_In_ WINHTTP_STATUS_CALLBACK lpfnInternetCallback, _Inout_ boost::system::error_code & ec)
                {
                    return set_status_callback(lpfnInternetCallback, WINHTTP_CALLBACK_FLAG_ALL_COMPLETIONS, ec);
                }

                // set option
                void set_option(_In_ DWORD dwOption, _In_ LPVOID lpBuffer, _In_ DWORD dwBufferLength, _Out_ boost::system::error_code & ec)
                {
                    bool ok = WinHttpSetOption(this->native_handle(), dwOption, lpBuffer, dwBufferLength);
                    if (!ok)
                    {
                        ec = boost::system::error_code(GetLastError(), boost::asio::error::get_system_category());
                    }
            ...
            public:
                ...
                void open(LPCWSTR agent, DWORD access_type, LPCWSTR proxy_name, LPCWSTR proxy_bypass, DWORD flags, boost::system::error_code & ec)
                {
                    HINTERNET hSession =
                ...
                {
                    open(L"WinASIO", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY, WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS,
            ...
            public:
                ...
                void connect(HINTERNET h_session, LPCWSTR url, INTERNET_PORT port, boost::system::error_code & ec)
                {
            ...
            public:
                ...
                void open(HINTERNET hConnect, LPCWSTR pwszVerb, LPCWSTR pwszObjectName, LPCWSTR pwszVersion, LPCWSTR pwszReferrer, LPCWSTR * ppwszAcceptTypes, DWORD dwFlags, boost::system::error_code & ec)
                {
                    HINTERNET hRequest =
                ...
                void open(HINTERNET hConnect, LPCWSTR method, LPCWSTR path, boost::system::error_code & ec)
                {
                ...
                void send(LPCWSTR lpszHeaders, DWORD dwHeadersLength, LPVOID lpOptional, DWORD dwOptionalLength, DWORD dwTotalLength, DWORD_PTR dwContext, boost::system::error_code & ec)
                {
                    bool ok = WinHttpSendRequest(parent_type::native_handle(), lpszHeaders, dwHeadersLength, lpOptional, dwOptionalLength, dwTotalLength, dwContext);
                    if (!ok)
                    {
                        ec = boost::system::error_code(GetLastError(), boost::asio::error::get_system_category());
                    }
                ...
                void query_headers(_In_ DWORD dwInfoLevel, _In_opt_ LPCWSTR pwszName, _Out_ LPVOID lpBuffer, _Inout_ LPDWORD lpdwBufferLength, _Inout_ LPDWORD lpdwIndex, _Inout_ boost::system::error_code & ec)
                {
                    bool ok =
                        WinHttpQueryHeaders(parent_type::native_handle(), dwInfoLevel, pwszName, lpBuffer, lpdwBufferLength, lpdwIndex);
                    if (!ok)
                    {
                        ec = boost::system::error_code(GetLastError(), boost::asio::error::get_system_category());
                    }
                ...
                void query_data_available(_Out_ LPDWORD lpdwNumberOfBytesAvailable, _Inout_ boost::system::error_code & ec)
                {
                    bool ok = WinHttpQueryDataAvailable(parent_type::native_handle(), lpdwNumberOfBytesAvailable);
                    if (!ok)
                    {
                        ec = boost::system::error_code(GetLastError(), boost::asio::error::get_system_category());
                    }
                ...
                void read_data(_Out_ LPVOID lpBuffer, _In_ DWORD dwNumberOfBytesToRead, _Out_ LPDWORD lpdwNumberOfBytesRead, _Inout_ boost::system::error_code & ec)
                {
                    bool ok = WinHttpReadData(parent_type::native_handle(), lpBuffer, dwNumberOfBytesToRead, lpdwNumberOfBytesRead);
                    if (!ok)
                    {
                        ec = boost::system::error_code(GetLastError(), boost::asio::error::get_system_category());
                    }
                ...
                void add_headers(_In_ LPCWSTR lpszHeaders, _In_ DWORD dwHeadersLength, _In_ DWORD dwModifiers, _Out_ boost::system::error_code & ec)
                {
                    bool ok =
                        WinHttpAddRequestHeaders(parent_type::native_handle(), lpszHeaders, dwHeadersLength, dwModifiers);
                    if (!ok)
                    {
                        ec = boost::system::error_code(GetLastError(), boost::asio::error::get_system_category());
                    }
            ...
            namespace header
            {
                ...
                template <typename Executor = net::any_io_executor>
                void get_status_code(basic_winhttp_request_handle<Executor> & h, _Out_ boost::system::error_code & ec, _Out_ DWORD & dwStatusCode)
                {
                    DWORD dwSize = sizeof(dwStatusCode);
                    // read status code as a number
                    h.query_headers(WINHTTP_QUERY_STATUS_CODE | WINHTTP_QUERY_FLAG_NUMBER, WINHTTP_HEADER_NAME_BY_INDEX, &dwStatusCode, &dwSize, WINHTTP_NO_HEADER_INDEX, ec);
                }

                // todo: make private
                template <typename Executor = net::any_io_executor>
                void get_string_header_helper(basic_winhttp_request_handle<Executor> & h, _In_ DWORD dwInfoLevel, _Out_ boost::system::error_code & ec, _Out_ std::wstring & data)
                {
                    DWORD dwSize;
                    // read version length
                    h.query_headers(dwInfoLevel, WINHTTP_HEADER_NAME_BY_INDEX, NULL, &dwSize, WINHTTP_NO_HEADER_INDEX, ec);
                    ...
                    ec.clear();
                    data.resize(dwSize);
                    h.query_headers(dwInfoLevel, WINHTTP_HEADER_NAME_BY_INDEX, data.data(), &dwSize, WINHTTP_NO_HEADER_INDEX, ec);
                ...
                template <typename Executor = net::any_io_executor>
                void get_version(basic_winhttp_request_handle<Executor> & h, _Out_ boost::system::error_code & ec, _Out_ std::wstring & data)
                {
                    get_string_header_helper(h, WINHTTP_QUERY_VERSION, ec, data);
                }

                template <typename Executor = net::any_io_executor>
                void get_content_type(basic_winhttp_request_handle<Executor> & h, _Out_ boost::system::error_code & ec, _Out_ std::wstring & data)
                {
                    get_string_header_helper(h, WINHTTP_QUERY_CONTENT_TYPE, ec, data);
                }

                template <typename Executor = net::any_io_executor>
                void get_all_raw_crlf(basic_winhttp_request_handle<Executor> & h, _Out_ boost::system::error_code & ec, _Out_ std::wstring & data)
                {
                    get_string_header_helper(h, WINHTTP_QUERY_RAW_HEADERS_CRLF, ec, data);
                }

                template <typename Executor = net::any_io_executor>
                void get_trailers(basic_winhttp_request_handle<Executor> & h, _Out_ boost::system::error_code & ec, _Out_ std::wstring & data)
                {
                ...
                template <typename Executor = net::any_io_executor>
                void add_header(basic_winhttp_request_handle<Executor> & h, _In_ std::wstring & data, _Out_ boost::system::error_code & ec)
                {
                ...
                {
                public:
                    accept_types(std::initializer_list<std::wstring> l)
                        : data_(l)
                    {
                    }
            ...
            namespace error
            {
                ...
                inline void get_secure_failure_err_str(_In_ DWORD code, _Out_ std::wstring & data)
                {
...
#define CASE_OF(constant) \
    case constant:        \
        return (L#constant)
                ...
                inline void get_api_error_str(_In_ WINHTTP_ASYNC_RESULT * pAR, _Out_ std::wstring & data)
                {
                    std::wstringstream error_ss;
