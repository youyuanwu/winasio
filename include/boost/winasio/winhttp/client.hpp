        ...
        namespace winhttp
        {
            ...
            namespace details
            {
                ...
                public:
                    ...
                    async_exec_op(basic_winhttp_request_asio_handle<executor_type> & h, DynamicBuffer & buff, LPCWSTR lpszHeaders, DWORD dwHeadersLength, LPVOID lpOptional, DWORD dwOptionalLength, DWORD dwTotalLength)
                        : h_(h)
                        , buff_(buff)
                        , lpszHeaders_(lpszHeaders)
                        , dwHeadersLength_(dwHeadersLength)
                        , lpOptional_(lpOptional)
                        , dwOptionalLength_(dwOptionalLength)
                        , dwTotalLength_(dwTotalLength)
                        , state_(state::idle)
                    {
                    }
                    ...
                    void operator()(Self & self, boost::system::error_code ec = {}, std::size_t len = 0)
                    {
                        ...
                        case state::idle:
                            state_ = state::send;
                            h_.async_send(lpszHeaders_, dwHeadersLength_, lpOptional_, dwOptionalLength_, dwTotalLength_, std::move(self));
                            break;
                        case state::send:
            ...
            void async_exec(payload & p, basic_winhttp_connect_handle<Executor> & h_connect, basic_winhttp_request_asio_handle<Executor> & h_request, DynamicBuffer & buffer, Token && token)
            {
                ...
                if (p.secure && p.insecure_skip_verify)
                {
                    ...
                    h_request.set_option(WINHTTP_OPTION_SECURITY_FLAGS, (PVOID) &dwSecurityFlags, sizeof(dwSecurityFlags), ec);
                    if (ec)
                    {
