...
TEST(HTTPClient, DISABLED_MyTest)
{
    ...
    rcContext.h_request.send(WINHTTP_NO_ADDITIONAL_HEADERS, 0, WINHTTP_NO_REQUEST_DATA, 0, 0, (DWORD_PTR) &rcContext, ec);
    ASSERT_FALSE(ec.failed());
    ...
    // check status
    DWORD dwStatusCode;
    winnet::winhttp::header::get_status_code(rcContext.h_request, ec, dwStatusCode);
    ...
    winnet::winhttp::header::get_content_type(rcContext.h_request, ec, content_type);
    ASSERT_EQ(boost::system::errc::success, ec);
